...
using EmbeddingResponse = SpongeEngine.GPT4AllSharp.Models.Embedding.EmbeddingResponse;
using Exception = SpongeEngine.GPT4AllSharp.Models.Exception;

namespace SpongeEngine.GPT4AllSharp.Providers.Betalgo
{
   public class BetalgoProvider : INativeProvider
   {
       ...
       public async Task<ModelsResponse> ListModelsAsync(CancellationToken cancellationToken = default)
       {
           ...
           {
               Data = response.Models.Select(m => new Gpt4AllSharpModel 
               { 
                   Id = m.Id,
                   Object = "model",
                   Type = "llm"
       ...
       public async Task<Gpt4AllSharpModel> GetModelAsync(string modelId, CancellationToken cancellationToken = default)
       {
           var response = await _client.Models.RetrieveModel(modelId, cancellationToken);
           return new Gpt4AllSharpModel
           {
               Id = response.Id,
               Object = "model",
               Type = "llm"
